...
def plot_time(results):
    ...
    plt.figure(figsize=(12, 6))
    plt.plot(df['timestamp'], df['game_result'])
    ...
    high_numbers = df[df['game_result'] > 60]
    print(f"Frequency of numbers > 60: {len(high_numbers)} out of {len(df)} ({len(high_numbers)/len(df)*100:.2f}%)")
    ...
    df['hour'] = df['timestamp'].dt.hour
    df['day_of_week'] = df['timestamp'].dt.dayofweek
    high_numbers_by_hour = df[df['game_result'] > 60].groupby('hour').size()
    high_numbers_by_day = df[df['game_result'] > 60].groupby('day_of_week').size()
    ...
    # 4. Moving average
    df['moving_avg'] = df['game_result'].rolling(window=50).mean()
    plt.figure(figsize=(12, 6))
    plt.plot(df['timestamp'], df['game_result'], alpha=0.5)
    plt.plot(df['timestamp'], df['moving_avg'], color='r')
    plt.axhline(y=60, color='g', linestyle='--')
    plt.title('Game Results with Moving Average')
    plt.xlabel('Timestamp')
    plt.ylabel('Game Result')
    plt.show()
